...
MedianWeightedPredictionErrorCompute <- function(file, problem){
    # Functions used to caculate median error
    #
    # Args:
    #  file: Weighted error results csv file which need to compute median error in each instance.
    #  problem: Problem maxsat and tsp.
    # 
    # Return: 
    #   Save median weightd perdiction in each instance in a csv file
    #
    
    if(grepl(problem, "maxsat")){
        data.matrix <- LoadMaxsat(file)
    }
    
    if(grepl(problem, "tsp")){
    ...
    savecsv.names = paste("./analysis-compare/res-h_percents_error_in_prediction/", problem, "_weighted_maxTrain_maxTest_single_run.csv" , sep = "")
    cat(savecsv.names, "\n")
...
problem = "tsp"
MedianWeightedPredictionErrorCompute(file, problem)
