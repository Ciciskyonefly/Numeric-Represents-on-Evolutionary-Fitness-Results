...
Intersect.plot <- function(formula, xData, yData, ppres){
        ...
        #
        # Returns:
        #   A numeric values that affects the residuals between yData and the predict yData. 
...
size = 10
#data.path = file.path
merge.data <- f_loop(file.path,size = size)
...
# data.names <- c('ts1e7r0f.gr21','ts1e7r0f.dantzig42','ts1e7r0f.att48')
...
for(i in 1:length(names(split.data))){
        ...
        for(j in 1:nrow(split.data[[i]])){
                ...
                plot.data <- Intersect.plot(model$formula,xData,yData,as.numeric(temp.data[j,c("a","b","c","d")]))
                ylim = c(min(plot.data$fyData),max(plot.data$fyData) + 1)
                        
                lines(plot.data[order(plot.data$xData),],col= j,lwd = 5)
        
                
                if(j == nrow(temp.data)){
                        legend("topright",legend=legend.names,
                               col=c(1:nrow(temp.data)), lty= 1,lwd = 5, cex=0.8)
                }
