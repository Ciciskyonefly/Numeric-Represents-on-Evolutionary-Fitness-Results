...
for(mod in 1:length(list.model)){
        ...
        for( j in 1: 5){
                ...
                temp = cbind( list.model[[mod]]$name, xgb_model$bestTune, sum(diag(yliu$table))/sum(sum(yliu$table)))
                
                modelresults = rbind(modelresults, temp)
                
        } 
        
                residuals <- mean(modelresults[,9])
...
# logisticModelpositive & 0.8183 \\
# expLinearModelpositive & 0.7433 \\
# decayModelpositive & 0.8083 \\
# decayModelnegative & 0.805 \\
